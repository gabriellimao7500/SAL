...
BEGIN
    ...
    WHILE data_atual <= data_fim DO
        ...
        DELETE FROM reserva
        WHERE dataReserva = data_atual + INTERVAL 4 DAY
          AND periodo = 'Manhã'
          AND aulaReserva = 1
          AND idLaboratorio = 1;
        INSERT INTO reserva (dataReserva, periodo, aulaReserva, idProfessor, idLaboratorio, motivo)
        ...
        WHERE dataReserva = data_atual + INTERVAL 3 DAY
          ...
          AND idLaboratorio = 3;
        INSERT INTO reserva (dataReserva, periodo, aulaReserva, idProfessor, idLaboratorio, motivo)
        VALUES (data_atual + INTERVAL 3 DAY, 'Manhã', 1, 3, 3, '1º MODULAR NUTRIÇÃO E DIETÉTICA - ANNIE');

        -- QUINTA (Laboratório 3 - Manhã) - Aula 2
        DELETE FROM reserva
        WHERE dataReserva = data_atual + INTERVAL 3 DAY
          AND periodo = 'Manhã'
          AND aulaReserva = 2
          AND idLaboratorio = 3;
        INSERT INTO reserva (dataReserva, periodo, aulaReserva, idProfessor, idLaboratorio, motivo)
        VALUES (data_atual + INTERVAL 3 DAY, 'Manhã', 2, 3, 3, '1º MODULAR NUTRIÇÃO E DIETÉTICA - ANNIE');
        ...
        WHERE dataReserva = data_atual + INTERVAL 3 DAY
          ...
          AND idLaboratorio = 3;
        INSERT INTO reserva (dataReserva, periodo, aulaReserva, idProfessor, idLaboratorio, motivo)
        VALUES (data_atual + INTERVAL 3 DAY, 'Manhã', 3, 3, 3, '1º MODULAR NUTRIÇÃO E DIETÉTICA - ANNIE');
        ...
        -- SEGUNDA (Laboratório 3 - Tarde) - Aula 1
        DELETE FROM reserva
        WHERE dataReserva = data_atual + INTERVAL 0 DAY
          AND periodo = 'Tarde'
          AND aulaReserva = 1
          AND idLaboratorio = 3;
        INSERT INTO reserva (dataReserva, periodo, aulaReserva, idProfessor, idLaboratorio, motivo)
        VALUES (data_atual + INTERVAL 0 DAY, 'Tarde', 1, 3, 3, 'SW II - 3º ENSINO MÉDIO COM HABILITAÇÃO PROFISSIONAL TÉCNICA EM INFORMÁTICA PARA INTERNET - MTEC  - EVERSON / CARLOS');

        -- SEGUNDA (Laboratório 3 - Tarde) - Aula 2
        DELETE FROM reserva
        WHERE dataReserva = data_atual + INTERVAL 0 DAY
          AND periodo = 'Tarde'
          AND aulaReserva = 2
